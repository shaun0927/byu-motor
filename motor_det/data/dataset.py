...
from motor_det.utils.augment import random_flip3d
...
class MotorTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 3) GT 필터 & 타겟 맵 생성
        mask = (
            (self.centers[:,2] >= z0) & (self.centers[:,2] < z1) &
            (self.centers[:,1] >= y0) & (self.centers[:,1] < y1) &
            (self.centers[:,0] >= x0) & (self.centers[:,0] < x1)
        )
